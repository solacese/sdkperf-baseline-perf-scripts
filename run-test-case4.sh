...
#
# Persistent publish to topic, read from queue load balanced across multiple connections.
# Subscription to queue must have been configured before running test.
# Queue must be configured as non-excusive!
#
...
$SDK_PERF -cip="$SMFS_URL" -cu=$MSG_USER@$VPN -cp=$MSG_PASS -ptl=perf-topic -sql=perf-queue -msa=1000 -mn=30000 -mt=persistent -mr=1000 -cc=5 -rc=10
